 ...
 */
function createSealIdentity(contractId: string, signerAddresses: string[]): Buffer {
  ...
  const normalizedSigners = signerAddresses.map((addr: string) => 
 ...
 */
function createSealIdentityHex(contractId: string, signerAddresses: string[]): string {
 ...
 */
export function usageSample(): void {
  /* 
